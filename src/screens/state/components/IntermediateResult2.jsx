  ...
  const getIntermediateResult2 = async (searchitem) => {
    try {
      const res = await axiosInstance.get(
        `/admin/indicators/intermediateresult2?state=${searchitem?.state}&lga=${searchitem?.lga}&healthfacility=${searchitem?.healthFacility}&from=${searchitem?.datefrom}&to=${searchitem?.dateto}&filter=${filteritem}`
      );
