  ...
  const getIntermediateResult1 = async (searchitem) => {
    try {
      const res = await axiosInstance.get(
        `/admin/indicators/intermediateresult1?state=${state}&lga=${lga}&healthfacility=${searchitem?.healthFacility}&from=${searchitem?.datefrom}&to=${searchitem?.dateto}&filter=${filteritem}`
      );
